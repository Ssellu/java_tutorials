...
>(1) 원소를 **삽입**하는 경우 
><pre>삽입할 위치가 맨 앞, 혹은 중간이라면<br>빈 칸을 만들기 위하여 삽입할 위치 이후의 원소들을 <b>하나씩 밀어내는</b> 재배치 작업이 필요합니다.<br>
>밀어내는 작업을 위해 loop를 진행해야 하는데요, 여기서 끝이 아닙니다. <br>
>마지막 칸까지 원소가 있는 완전 포화 배열에는 단순하게 밀어내기만 한다면 마지막 원소는 사라지게 되어있지요. <br>따라서 이를 방지하는 추가 분기문이 필요합니다.</pre>
